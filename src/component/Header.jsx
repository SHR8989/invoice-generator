      ...
      <div className="flex items-center space-x-1 md:space-x-4">
        <Menu as="div" className="relative">
          <Menu.Button className="flex items-center sapce-x-1 md:space-x-2 text-sm md:text-lg text-white hover:text-violet-400 focus:outline-none">
            <Filter size={20} />
            <span className="w-13">Filter by Status</span>
          </Menu.Button>
              ...
              <Menu.Item key={s}>
                {({ active }) => (
                  <button
                    className={`${
                      active ? "bg-slate-700" : ""
                    } w-full text-left md:px-4 py-2 rounded-lg capitalize ${filter === s ? "text-violet-500"
                        : "text-white"
        ...
        <button
          type="button"
          className="bg-violet-500 hover:bg-violet-600 text-white pl-2 md:px-6 py-2 rounded-3xl md:rounded-full flex items-center md:space-x-2"
